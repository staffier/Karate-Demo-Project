...
class GatlingWithKarate extends Simulation {
  ...
  val happyPath = scenario("Happy Path Test")
    .exec(karateFeature("classpath:examples/tests/performance-test.feature@happy-path"))

  val sadPath = scenario("Sad Path Test")
    .exec(karateFeature("classpath:examples/tests/performance-test.feature@not-so-happy-path"))

  val optionalFields = scenario("Optional Fields")
    .exec(karateFeature("classpath:examples/tests/performance-test.feature@optional-fields"))

  val allScenarios = scenario("All Scenarios")
    .exec(karateFeature("classpath:examples/tests/performance-test.feature"))
